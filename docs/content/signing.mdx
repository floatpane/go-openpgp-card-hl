...
- **git commit/tag signing** — git hands the object to the signing program on
  ...
  `Sign`.
- **`multipart/signed` mail (PGP/MIME)** — use `SignMIME` instead. It accepts
  the full raw MIME message and returns a complete `multipart/signed` envelope,
  handling boundary generation, MIME part structure, and armor internally. See
  [PGP/MIME](./mime).
